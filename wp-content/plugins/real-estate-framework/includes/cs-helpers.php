...
if ( ! function_exists( 'wp_rem_cs_input_clean' ) ) {
	...
	function wp_rem_cs_input_clean( $source, $type = 'string' ) {
		...
		switch ( strtoupper( $type ) ){
			...
			case 'WORD':
                $result = (string) preg_replace_callback( '/[^A-Z_]/i', function($m){return '';}, $source );
                break;

            case 'ALNUM':
                $result = (string) preg_replace_callback( '/[^A-Z0-9]/i', function($m){return '';}, $source );
                break;

            case 'CMD':
                $result = (string) preg_replace_callback( '/[^A-Z0-9_\.-]/i', function($m){return '';}, $source );
                $result = ltrim( $result, '.' );
                break;
			...
			case 'USERNAME':
                $result = (string) preg_replace_callback( '/[\x00-\x1F\x7F<>"\'%&]/', function($m){return '';}, $source );
                break;
