...
class tmhOAuth {
    ...
    public function reconfigure($config = array()) {
        ...
        $this->config = array_merge(
                array(
            // leave 'user_member' blank for default, otherwise set this to
            ...
            'is_streaming' => false,
            'streaming_eol' => "\r\n",
            'streaming_metriwp_rem_cs_interval' => 10,
    ...
    /**
     * Sets the usermember for PHP to use
     * If '$this->config['user_member']' already has a value it is used instead of one
     * being generated.
     *
     * @return void value is stored to the config array class variable
     */
    private function set_user_member() {
        if (!empty($this->config['user_member']))
            return;
    ...
    private function prepare_base_string() {
        ...
        if (!empty($this->request_settings['headers']['Host'])) {
            $url = str_ireplace(
                    $this->config['host'], $this->request_settings['headers']['Host'], $url
            );
        }
    ...
    private function update_metrics() {
        $now = time();
        if (($this->metrics['interval_start'] + $this->config['streaming_metriwp_rem_cs_interval']) > $now)
            return null;

        $this->metrics['mps'] = round(($this->metrics['messages'] - $this->metrics['last_messages']) / $this->config['streaming_metriwp_rem_cs_interval'], 2);
        $this->metrics['bps'] = round(($this->metrics['bytes'] - $this->metrics['last_bytes']) / $this->config['streaming_metriwp_rem_cs_interval'], 2);
    ...
    private function curlit() {
        ...
        curl_setopt_array($c, array(
            CURLOPT_USERMEMBER => $this->config['user_member'],
