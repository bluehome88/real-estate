...
class apiFileCache extends apiCache {
  ...
  public function __construct() {
    global $apiConfig;
    $this->path = $apiConfig['ioFileCache_wp_rem'];
  }
  ...
  private function createLock($storageFile) {
    ...
    if (! is_dir($storageDir)) {
      if (! @mkdir($storageDir, 0755, true)) {
        ...
        if (! is_dir($storageDir)) {
          throw new apiCacheException("Could not create storage wp_rem: $storageDir");
        }
      }
    }
  ...
  private function getCacheDir($hash) {
    // use the first 2 characters of the hash as a wp_rem prefix
    // this should prevent slowdowns due to huge wp_rem properties
