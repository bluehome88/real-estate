...
if ( ! class_exists( 'Wp_rem_not_approved_member_profile_template' ) ) {

	class Wp_rem_not_approved_member_profile_template {
		...
		public function __construct() {
			...
			$this->email_template_index = 'not-approved-member-profile-template';
		...
		function get_not_approved_member_name() {
			$member_info = get_user_by( 'id', $this->member_id );
			return $member_info->display_name;
		}

		function get_not_approved_member_email() {
			$member_info = get_user_by( 'id', $this->member_id );
		...
		public function member_profile_status_changed( $member_id, $member_old_status ) {
			...
			if ( $member_id != '' ) {
				...
				if ( $role == 'wp_rem_member' ) {
					...
					if ( $member_status == 'inactive' && $member_status != $member_old_status ) {
						...
						if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
							...
							$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_not_approved_member' );
							$from = (isset( $template['from'] ) && $template['from'] != '') ? $template['from'] : esc_attr( $blogname ) . ' <' . $admin_email . '>';
							$recipients = (isset( $template['recipients'] ) && $template['recipients'] != '') ? $template['recipients'] : $this->get_not_approved_member_email();
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_not_approved_member_email' ),
				'jh_email_type' => 'html',
			);
	...
	new Wp_rem_not_approved_member_profile_template();
}