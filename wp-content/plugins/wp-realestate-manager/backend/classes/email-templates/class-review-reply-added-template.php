<?php 
 ...
 /**
 * Review Reply Added Email Template.
 *
 * @since 1.0
 * @package	Homevillas
 */

if ( ! class_exists( 'Wp_rem_review_reply_added_email_template' ) ) {

	class Wp_rem_review_reply_added_email_template {
		...
		public function __construct() {
			$this->user = array();
			$this->email_template_type = 'Review Reply Added';
			...
			$this->email_template_index = 'review-reply-added-template';
			...
			add_action( 'wp_rem_review_reply_added_email', array( $this, 'wp_rem_review_reply_added_email_callback' ), 10, 2 );
		}

		public function wp_rem_review_reply_added_email_callback( $user = '', $review_id = '' ) {

			$this->user = $user;
			$this->review_id = $review_id;
                        $reciever_email = '';
			$template = $this->get_template();
                        $review_reply_parent    = get_post_meta( $review_id, 'wp_rem_parent_review', true );
                        
                        
                        $reciever_email = get_post_meta( $review_reply_parent, 'user_email', true );
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_review_reply_added' );
				...
				Wp_rem_review_reply_added_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_review_reply_added_email' ),
				'jh_email_type' => 'html',
			);
	...
	new Wp_rem_review_reply_added_email_template();
}
