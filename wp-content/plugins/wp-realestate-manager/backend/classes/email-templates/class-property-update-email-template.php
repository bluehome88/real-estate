<?php 
 ...
 /**
 * Property Update Email Templates
 *
 * @since 1.0
 * @package	Homevillas
 */

if ( ! class_exists( 'Wp_rem_property_update_email_template' ) ) {

	class Wp_rem_property_update_email_template {
		...
		public function __construct( $args = array() ) {
			...
			$this->email_template_index = 'property-update-email-template';

			add_filter( 'wp_rem_email_template_settings', array( $this, 'template_settings_callback' ), 12, 1 );

			add_action( 'wp_rem_property_updated_on_front', array( $this, 'wp_rem_property_updated_callback' ), 10, 1 );

			add_action( 'wp_rem_property_updated_on_admin', array( $this, 'wp_rem_property_updated_callback' ), 10, 2 );
		...
		public function wp_rem_property_updated_callback( $user, $property_id ) {
			...
			if ( $property_id != '' ) {
				...
				if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
					...
					$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_property_updated' );
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_property_updated_email' ),
				'jh_email_type' => 'html',
			);
	...
	return new Wp_rem_property_update_email_template();
}