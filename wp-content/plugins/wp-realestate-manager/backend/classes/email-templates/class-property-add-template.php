...
if ( ! class_exists( 'Wp_rem_property_add_email_template' ) ) {

	class Wp_rem_property_add_email_template {
			...
			$this->email_template_variables = array(
				...
				array(
					...
					'value_callback' => array( $this, 'get_property_added_user_name' ),
                                ...
                                array(
					'tag' => 'PROPERTY_COMPANY_NAME',
					'display_text' => 'Property Company Name',
					'value_callback' => array( $this, 'get_property_added_company_name' ),
				),
		...
		public function wp_rem_property_add_email_callback( $user = '', $property_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_property_added' );
				$from = (isset( $template['from'] ) && $template['from'] != '') ? $template['from'] : esc_attr( $blogname ) . ' <' . $admin_email . '>';
				$recipients = (isset( $template['recipients'] ) && $template['recipients'] != '') ? $template['recipients'] : $this->get_property_added_email();
				...
				Wp_rem_property_add_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_property_added_email' ),
				'jh_email_type' => 'html',
			);
		...
		function get_property_added_user_name() {
			...
			return $user_name;
                ...
                function get_property_added_company_name(){
                    $user_id = $this->user->ID;
                    $user_data  = get_user_info_array( $user_id );
                    return isset( $user_data['display_name'] )? $user_data['display_name']: '';
                }
	...
	new Wp_rem_property_add_email_template();
}
