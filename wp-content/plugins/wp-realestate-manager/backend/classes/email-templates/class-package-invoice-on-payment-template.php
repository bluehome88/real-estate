...
if ( ! class_exists( 'Wp_rem_package_user_invoice_template' ) ) {

	class Wp_rem_package_user_invoice_template {
		...
		public function __construct() {
			...
			$this->email_template_variables = array(
				...
				array(
					'tag' => 'PACKAGE_PAYMENT_INVOICE',
					'display_text' => 'Package Payment Invoice',
					'value_callback' => array( $this, 'get_package_payment_invoice' ),
				),
			);
			$this->template_group = 'Package';

			$this->email_template_index = 'package-invoice-on-payment-template';
			...
			add_action( 'wp_rem_package_invoice_on_payment_email', array( $this, 'wp_rem_package_invoice_on_payment_email_callback' ), 10, 2 );
		}

		public function wp_rem_package_invoice_on_payment_email_callback( $user = '', $package_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_package_used' );
				...
				Wp_rem_package_user_invoice_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_package_used_email' ),
				'jh_email_type' => 'html',
			);
		...
		function get_package_payment_invoice() {
			return 'here goes the invoice for package: ' . $this->get_package_title();
		}

	}

	new Wp_rem_package_user_invoice_template();
}
