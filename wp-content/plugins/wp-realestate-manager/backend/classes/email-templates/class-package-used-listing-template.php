<?php 
 ...
 /**
 * Package Used Email Template.
 *
 * @since 1.0
 * @package	Homevillas
 */

if ( ! class_exists( 'Wp_rem_package_used_email_template' ) ) {

	class Wp_rem_package_used_email_template {
		...
		public function __construct() {
			...
			$this->email_template_index = 'package-used-property-template';
			...
			add_action( 'wp_rem_package_user_property_email', array( $this, 'wp_rem_package_user_property_email_callback' ), 10, 2 );
		}

		public function wp_rem_package_user_property_email_callback( $user = '', $package_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_packged_used_property' );
				...
				Wp_rem_package_used_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_packged_used_property_email' ),
				'jh_email_type' => 'html',
			);
	...
	new Wp_rem_package_used_email_template();
}
