<?php 
 ...
 /**
 * Property Pending Approval Email Templates.
 *
 * @since 1.0
 * @package	Homevillas
 */

if ( ! class_exists( 'Wp_rem_property_pending_email_template' ) ) {

	class Wp_rem_property_pending_email_template {
		...
		public function __construct( $args = array() ) {
			...
			$this->email_template_index = 'property-pending-email-template';

			add_filter( 'wp_rem_email_template_settings', array( $this, 'template_settings_callback' ), 12, 1 );

			add_action( 'wp_rem_property_pending_email', array( $this, 'wp_rem_property_pending_approval_callback' ), 10, 2 );
		...
		public function wp_rem_property_pending_approval_callback( $user, $property_id ) {
			...
			if ( $property_id != '' ) {
				...
				if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
					...
					$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_property_pending' );
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_property_pending_email' ),
				'jh_email_type' => 'html',
			);
	...
	return new Wp_rem_property_pending_email_template();
}
