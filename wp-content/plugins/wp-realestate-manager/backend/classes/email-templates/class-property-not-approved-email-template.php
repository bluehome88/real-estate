...
if ( ! class_exists( 'Wp_rem_property_not_approved_email_template' ) ) {

	class Wp_rem_property_not_approved_email_template {
		...
		public function __construct() {
			...
			$this->email_template_variables = array(
				...
				array(
					...
					'value_callback' => array( $this, 'get_property_not_approved_title' ),
				),
				array(
					'tag' => 'PROPERTY_LINK',
					'display_text' => 'Property Link',
					'value_callback' => array( $this, 'get_property_not_approved_link' ),
				),
                                array(
					'tag' => 'PROPERTY_TYPE',
					...
					'value_callback' => array( $this, 'get_property_not_approved_user_name' ),
				),
				array(
					'tag' => 'PROPERTY_USER_EMAIL',
					'display_text' => 'Property User Email',
					'value_callback' => array( $this, 'get_property_not_approved_user_email' ),
				),
			);

			$this->template_group = 'Property';
			$this->email_template_index = 'property-not-approved-template';

			add_filter( 'wp_rem_email_template_settings', array( $this, 'template_settings_callback' ), 12, 1 );

			// Add action property status callback
			add_action( 'wp_rem_property_not_approved_email', array( $this, 'property_status_changed_callback' ), 10, 2 );
		...
		function get_property_not_approved_user_name() {
			$user_name = $this->user->display_name;
			return $user_name;
		}

		function get_property_not_approved_user_email() {
			$email = $this->user->user_email;
			return $email;
		}

		function get_property_not_approved_title() {
			return get_the_title( $this->property_id );
		}

		function get_property_not_approved_link() {
		...
		public function property_status_changed_callback( $user, $property_id ) {
			...
			if ( $property_id != '' ) {
				...
				if ( isset( $template['email_notification'] ) && $template['email_notification'] ) {
					...
					$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_rem_plugin_text_srt( 'wp_rem_property_not_approved' );
					$from = (isset( $template['from'] ) && $template['from'] != '') ? $template['from'] : esc_attr( $blogname ) . ' <' . $admin_email . '>';
					$recipients = (isset( $template['recipients'] ) && $template['recipients'] != '') ? $template['recipients'] : $this->get_property_not_approved_user_email();
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_rem_plugin_text_srt( 'wp_rem_property_not_approved_email' ),
				'jh_email_type' => 'html',
			);
	...
	new Wp_rem_property_not_approved_email_template();
}