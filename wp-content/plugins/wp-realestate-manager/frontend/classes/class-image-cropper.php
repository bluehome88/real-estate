 ...
 */
if (!class_exists('Wp_rem_Image_Cropper')) {

    class Wp_rem_Image_Cropper {
        ...
        public function save_iamge_to_file_callback() {
            ...
            //Check write Access to Homevillas

            if (!is_writable($imagePath)) {
                $response = Array(
                    "status" => 'error',
                    "message" => wp_rem_plugin_text_srt('wp_rem_member_no_permissions_to_upload'),
                );
            ...
            if (in_array($extension, $allowedExts)) {
                if ($_FILES["img"]["error"] > 0) {
                    $response = array(
                        "status" => 'error',
                        "message" => wp_rem_plugin_text_srt('wp_rem_image_cropper_error_return_code'). ': ' . $_FILES["img"]["error"],
                    );
                } else {
                ...
                $response = array(
                    "status" => 'error',
                    "message" => wp_rem_plugin_text_srt('wp_rem_cropping_file_error'),
                );
            }
        ...
        public function image_crop_to_file_callback() {
            ...
            $wp_rem_absolute_path=$upload_dir_info['path'];
            $wp_rem_unlink_image_path= $wp_rem_absolute_path.'/'.$image_name;
           ...
           // wp_die();
            ...
            switch (strtolower($filetype['type'])) {
                ...
                case 'image/gif':
                    ...
                    $type = '.gif';
                    break;
                default: die(wp_rem_plugin_text_srt('wp_rem_image_cropper_type_not_supported'));
            }
            ...
            if (!is_writable(dirname($output_filename))) {
                $response = Array(
                    "status" => 'error',
                    "message" => wp_rem_plugin_text_srt('wp_rem_image_cropper_can_not_write_file')
                );
            } else {
                ...
                unlink($wp_rem_unlink_image_path);
            }
            print json_encode($response);
            wp_die();
        }
    ...
    global $Wp_rem_Image_Cropper;
    $Wp_rem_Image_Cropper = new Wp_rem_Image_Cropper();
}