...
if (!class_exists('Wp_rem_Member_Viewings')) {
    ...
    class Wp_rem_Member_Viewings {
        ...
        public function wp_rem_member_viewings_callback($member_id = '') {
            ...
            echo force_balance_tags($this->render_view_viewings($viewing_query, 'my', $member_user_type));
            wp_reset_postdata();
            wp_die();
        }
        ...
        public function wp_rem_member_received_viewings_callback($member_id = '') {
            ...
            $viewing_query = new WP_Query($args);
            echo force_balance_tags($this->render_view_viewings($viewing_query, 'received', $member_user_type));
            wp_reset_postdata();
            wp_die();
        }

        public function render_view_viewings($viewing_query = '', $type = 'my', $member_user_type = '') {
            ?>
            ...
            while ($viewing_query->have_posts()) : $viewing_query->the_post();
                ...
                <li class="<?php echo esc_html($read_unread); ?>">
					<div class="orders-title">
                        <h6 class="order-title"><a href="javascript:void(0);" onclick="javascript:wp_rem_arrange_viewing_detail('<?php the_ID(); ?>','<?php echo esc_html($type); ?>');"><?php echo wp_trim_words(get_the_title($viewing_property_id), 6, '...'); ?></a><span>( #<?php echo get_the_ID(); ?> )</span></h6>
                    </div>
