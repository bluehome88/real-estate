...
if ( ! class_exists('Wp_rem_Shortcode_Properties_Frontend') ) {
    ...
    class Wp_rem_Shortcode_Properties_Frontend {
        ...
        public function wp_rem_properties_content($property_arg = '') {
            ...
            // If this query is without left filters then filter for polygon over here else polgon filter is applied inside left filter.
            if ( isset($_REQUEST['loc_polygon_path']) && $_REQUEST['loc_polygon_path'] != '' && empty($all_post_ids) ) {
                $loc_polygon_path = $_REQUEST['loc_polygon_path'];
                $property_loop_obj->posts = $this->property_polygon_filter($loc_polygon_path, $property_loop_obj->posts);
                if ( empty($property_loop_obj->posts) ) {
                    $property_loop_obj->posts = array();
                }
            }

            if ( $property_loop_obj->post_count != 0 ) {
                $draw_post_ids = array();
                $draw_post_ids = array_values($property_loop_obj->posts);
                if ( empty($draw_post_ids) ) {
                    $draw_post_ids[] = 0;
                }
                $new_args = array(
                    'posts_per_page' => $posts_per_page,
                    'post__in' => $draw_post_ids,
                    'paged' => $_REQUEST[$paging_var],
                    'post_type' => 'properties',
                    'order' => $qryvar_property_sort_type,
                    'orderby' => $qryvar_sort_by_column,
                ...
                $property_loop_obj = wp_rem_get_cached_obj('property_result_cached_loop_obj1', $new_args, 12, false, 'wp_query');
            }
                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <div class="element-title <?php echo ($properties_title_alignment); ?>">
                                    ...
                                    if ( $properties_title != '' || $properties_subtitle != '' ) {
                                        ...
                                        if( $property_view == 'grid-classic' || $property_view == 'grid-default' ){
                                            ?>
                        ...
                        if ( ! empty($element_property_layout_switcher_view) && is_array($element_property_layout_switcher_view) ) {
                            ...
                            foreach ( $element_property_layout_switcher_view as $single_layout_view ) {
                                ...
                                if ( empty($view) && $views_counter === 0 ) {
                                    ...
                                    ?>
                                    <li><a href="javascript:void(0);" <?php if ( $view == $single_layout_view ) echo 'class="active"'; ?> <?php if ( $view != $single_layout_view ) { ?> onclick="wp_rem_property_view_switch('<?php echo esc_html($single_layout_view) ?>', '<?php echo esc_html($property_short_counter); ?>', '<?php echo esc_html($counter); ?>');"<?php } ?>><?php echo force_balance_tags($icon); ?></a></li>
                                    <?php
                                }
                                $views_counter ++;
                            }
...
//				}
        ...
        public function wp_rem_property_pagination_callback($args) {
            ...
            extract($args);
            $split_map      = ( isset( $split_map ) && $split_map == true )? 'split_map' :'';
